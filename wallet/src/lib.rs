...
/// Structure pour les dérivations de clés partagées (utilisée pour la confidentialité et la vérification des transactions).
...
impl SharedKeyDerivations {
  ...
  /// Calcule un hash d’unicité basé sur les key images des inputs (utilisé pour détecter les doubles dépenses).
